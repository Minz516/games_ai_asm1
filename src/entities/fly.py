...
from settings import (
    SEP_RADIUS, WIDTH, HEIGHT, WHITE, YELLOW, PURPLE,
...
class Fly:
    def __init__(self, pos):
        ...
        # Debug-only: last boids component vectors, for the F1 overlay.
        self.dbg_sep = V2()
        self.dbg_coh = V2()
        self.dbg_ali = V2()
    ...
    def update(self, dt, flies, frog, bounds_rect, bubbles):
        ...
        BubbleFleeRange = FLY_BUBBLE_FLEE_RANGE     # panic if bubble comes within this range
        StopFleeingRange = FLY_STOP_FLEEING_RANGE   # calm down when both frog and bubbles are beyond this
        IdleDistance = FLY_IDLE_DISTANCE            # far enough to consider idling
        IdleDelay    = FLY_IDLE_DELAY               # seconds of safety before entering Idle

        # Triggers based on the frog and bubbles
        dist_to_frog = (frog.pos - self.pos).length()
        scared_by_frog   = dist_to_frog < FLY_SCARE_BY_FROG_RANGE
        ...
        if self.state == FlyState.Flock:
            ...
            sep = boids_separation(self.pos, neighbors, sep_radius=SEP_RADIUS)
            coh = boids_cohesion(self.pos, neighbors)
            ali = boids_alignment(self.vel, neighbors)
            self.dbg_sep, self.dbg_coh, self.dbg_ali = sep, coh, ali
